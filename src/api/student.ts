...
import type { ApiResponse, PaginationRequest, Student } from "@/types/api";
...
export class StudentApi extends BaseApi {
  ...
  static async updateProfile(
    ...
    return this.put<Student>("/auth/student/update", formData, {
      requiresAuth: true,
    });
  }
  ...
  static async changePassword(passwordData: {
    ...
    return this.post<null>("/student/profile/change-password", passwordData, {
      requiresAuth: true,
    });
  }
  ...
  > {
    ...
    const url = queryString
      ? `/student/courses/available?${queryString}`
   ...
   */
  static async enrollInCourse(courseId: string): Promise<
    ApiResponse<{
      enrollment_date: string;
      course_id: string;
      student_id: string;
  ...
  static async unenrollFromCourse(
    ...
    return this.delete<null>(`/student/courses/${courseId}/enroll`, {
      requiresAuth: true,
    });
  }
  ...
  > {
    ...
    const queryString = params.toString();
    const url = queryString
      ? `/student/courses/${courseId}/lessons?${queryString}`
   ...
   */
  static async markLessonComplete(lessonId: string): Promise<
    ApiResponse<{
      lesson_id: string;
      completed_at: string;
      progress_percentage: number;
  ...
  > {
    return this.put<{
      ...
      time_spent_minutes: number;
      last_accessed: string;
    }>(`/student/lessons/${lessonId}/progress`, progressData, {
      requiresAuth: true,
    });
  }
   ...
   */
  static async getLessonQuiz(lessonId: string): Promise<
    ApiResponse<{
      id: string;
      title: string;
      description: string;
   ...
   */
  static async getQuizHistory(quizId: string): Promise<
    ApiResponse<
      Array<{
        attempt_number: number;
        score: number;
   ...
   */
  static async getRecommendedCourses(limit: number = 10): Promise<
    ApiResponse<
      Array<{
        id: string;
        title: string;
  ...
  > {
    ...
    >(`/student/recommendations/courses?limit=${limit}`, {
      requiresAuth: true,
    });
  }
   ...
   */
  static async getRecommendedLessons(limit: number = 5): Promise<
    ApiResponse<
      Array<{
        id: string;
        title: string;
  ...
  > {
    ...
    >(`/student/recommendations/lessons?limit=${limit}`, {
      requiresAuth: true,
    });
  }
   ...
   */
  static async getCertificates(pagination?: PaginationRequest): Promise<
    ApiResponse<
      Array<{
        id: string;
        course_id: string;
  ...
  > {
    ...
    const queryString = params.toString();
    const url = queryString
      ? `/student/certificates?${queryString}`
   ...
   */
  static async downloadCertificate(certificateId: string): Promise<
    ApiResponse<{
      download_url: string;
      expires_at: string;
    }>
