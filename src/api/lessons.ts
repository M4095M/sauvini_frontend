...
export class LessonsApi extends BaseApi {
  ...
  static async getModule(moduleId: string): Promise<ApiResponse<Module>> {
    try {
      return await this.get<Module>(`/courses/module/${moduleId}`, {
        requiresAuth: true,
      });
    } catch (error) {
   ...
   */
  static async getModuleProgress(moduleId: string): Promise<
    ApiResponse<{
      progress: number;
  ...
  > {
    try {
      return await this.get<{
        ...
        completedLessons: number;
        totalLessons: number;
      }>(`/progress/module/${moduleId}/progress`, { requiresAuth: true });
    } catch (error) {
  ...
  static async getChapter(chapterId: string): Promise<ApiResponse<Chapter>> {
    try {
      ...
      return await this.get<Chapter>(`/courses/chapter/${cleanChapterId}`, {
        requiresAuth: true,
      });
    } catch (error) {
  ...
  static async getLessons(chapterId: string): Promise<ApiResponse<Lesson[]>> {
    try {
      return await this.get<Lesson[]>(`/courses/chapter/${chapterId}/lessons`, {
        requiresAuth: false,
      });
    } catch (error) {
  ...
  static async markLessonComplete(
    lessonId: string
  ): Promise<ApiResponse<{ success: boolean; progress: LessonProgress }>> {
    try {
  ...
  > {
    try {
      return await this.post<{
        ...
        correctAnswers: Record<string, string | number>;
        explanations: Record<string, string>;
      }>(`/quizzes/${quizId}/submit`, { answers }, { requiresAuth: true });
    } catch (error) {
