...
export class AuthApi extends BaseApi {
  ...
  static async loginStudent(
    ...
    return this.post<LoginResponse<Student>>("/auth/student/login", loginData, {
      requiresAuth: false,
    });
  }
   ...
   */
  static async resetPasswordStudent(
    token: string,
    newPassword: string
  ): Promise<ApiResponse<null>> {
    return this.post<null>(
   ...
   */
  static async loginProfessor(
    credentials: LoginRequest
  ): Promise<ApiResponse<LoginResponse<Professor>>> {
    const loginData = {
      ...credentials,
      user_type: "professor",
    };
   ...
   * @returns Promise with reset request result
   */
  static async forgotPasswordProfessor(
    email: string
  ): Promise<ApiResponse<null>> {
    return this.post<null>(
   ...
   * @returns Promise with reset result
   */
  static async resetPasswordProfessor(
    token: string,
    newPassword: string
  ): Promise<ApiResponse<null>> {
    return this.post<null>(
  ...
  static async loginAdmin(
    ...
    return this.post<LoginResponse<Admin>>("/auth/admin/login", loginData, {
      requiresAuth: false,
    });
  }
  ...
  static async registerStudent(
    ...
    // For Django backend, we need to send JSON data with password_confirm
    const registrationData = {
      ...data,
      password_confirm: data.password, // Django backend expects password_confirm
    };

    // If we have a profile picture, we need to use FormData
    if (profilePicture) {
      const formData = new FormData();
      formData.append("student", JSON.stringify(registrationData));
      formData.append("profile_picture", profilePicture);

      return this.post<Student>("/auth/student/register", formData, {
        requiresAuth: false,
      });
    }

    // Otherwise, send as JSON
    return this.post<Student>("/auth/student/register", registrationData, {
      requiresAuth: false,
    });
  }
  ...
  static async registerProfessor(
    ...
    return this.post<Professor>("/auth/professor/register", formData, {
      requiresAuth: false,
    });
  }
  ...
  static async refreshAuthTokens(
    ...
    const response = await this.post<{ token: TokenPair; user: any }>(
      refreshEndpoint,
  ...
  static async logout(): Promise<ApiResponse<null>> {
    try {
      // Get current tokens to send refresh token for blacklisting
      ...
      if (refreshToken) {
        ...
        // No refresh token available, just clear frontend tokens
        BaseApi.clearTokens();
        return {
          success: true,
          message: "Logout successful (no backend token to blacklist)",
          data: null,
      ...
      // Always clear tokens from frontend
      BaseApi.clearTokens();
    }
  }
   ...
   * @returns Promise with logout result
   */
  static async logoutAllDevices(): Promise<ApiResponse<null>> {
    try {
      const response = await this.post<null>(
   ...
   * @returns Promise with reset request result
   */
  static async forgotPasswordAdmin(
    data: ForgotPasswordRequest
  ): Promise<ApiResponse<null>> {
    return this.post<null>("/auth/admin/forgot-password", data, {
      requiresAuth: false,
    });
  }
   ...
   */
  static async resetPasswordAdmin(
    data: ResetPasswordConfirmRequest
  ): Promise<ApiResponse<null>> {
    return this.post<null>("/auth/admin/reset-password", data, {
      requiresAuth: false,
    });
  }
  ...
  static async forgotPassword(
    ...
    return this.post<null>("/auth/forgot-password", data, {
      requiresAuth: false,
    });
  }
  ...
  static async resetPasswordConfirm(
    ...
    return this.post<null>("/auth/reset-password-confirm", data, {
      requiresAuth: false,
    });
  }
  ...
  static async verifyEmail(
    ...
    return this.post<null>("/auth/verify-email", data, {
      requiresAuth: false,
    });
  }
   ...
   */
  static async sendStudentVerificationEmail(
    email: string
  ): Promise<ApiResponse<null>> {
    return this.post<null>(
      "/auth/student/send-verification-email",
      { email, user_type: "student" },
      { requiresAuth: false }
    );
  }
  ...
  static async resendEmailVerification(
    ...
    console.warn(
      "resendEmailVerification is deprecated. Use sendStudentVerificationEmail instead."
    );
  ...
  static async getPendingProfessors(): Promise<ApiResponse<Professor[]>> {
    console.warn(
      "getPendingProfessors is deprecated. Use getAllProfessors with status filtering instead."
    );
