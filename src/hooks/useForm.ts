...
export function useForm<T extends Record<string, any>>({
  ...
  const registerFile = useCallback(
    (name: keyof T) => ({
      ref: (el: HTMLInputElement | null) => {
        // Don't store file inputs in refs, only in fileRefs
        // refs.current[name as string] = el;
      },
      name: name as string,
      onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        console.log(`File stored for ${String(name)}:`, file);
      },
    }),
    []
  );
  ...
  const getValues = useCallback((): Partial<T> => {
    ...
    // Get file values
    console.log("fileRefs.current:", fileRefs.current);
    Object.keys(fileRefs.current).forEach((key) => {
      const file = fileRefs.current[key];
      console.log(`File for ${key}:`, file);
