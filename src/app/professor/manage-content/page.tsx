  ...
  const fetchModules = async () => {
    try {
      ...
      if (response.success && response.data) {
        // Fetch chapter counts for each module
        const modulesWithChapterCounts = await Promise.all(
          response.data.map(async (module) => {
            try {
              // Pass just the module ID, the API will add the Module: prefix
              ...
              if (chaptersResponse.success && chaptersResponse.data) {
                return {
                  ...module,
                  chapters: chaptersResponse.data.map((chapter) =>
                    ChaptersApi.transformChapter(chapter, module.id)
                  ),
                };
              }
            } catch (error) {
            ...
            return module; // Return module with empty chapters array if fetch fails
          })
        );

        setModules(modulesWithChapterCounts);
      } else {
