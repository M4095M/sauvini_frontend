...
export default function ResetPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!newPassword || newPassword.length < 8) {
      newErrors.newPassword =
        t("auth.reset.errors.password_min") ||
        "Password must be at least 8 characters";
    }

    if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(newPassword)) {
      newErrors.newPassword =
        t("auth.reset.errors.password_format") ||
        "Password must contain uppercase, lowercase, and number";
    }

    if (!confirmPassword) {
      newErrors.confirmPassword =
        t("auth.reset.errors.confirm_required") ||
        "Please confirm your password";
    } else if (confirmPassword !== newPassword) {
      newErrors.confirmPassword =
        t("auth.reset.errors.password_mismatch") || "Passwords do not match";
    }
    ...
    try {
      ...
      const errorMessage =
        error?.message ||
        t("auth.reset.errors.reset_failed") ||
  ...
  const SuccessContent = ({ className = "" }: { className?: string }) => (
    <div className={`flex flex-col items-center text-center ${className}`}>
      ...
      <h2
        className={`mt-8 text-2xl sm:text-3xl font-extrabold ${
      ...
      <div className="mt-8">
        <Link href="/auth/login" className="block">
          <Button
            ...
            size="M"
            icon_position="none"
            text={t("auth.reset.successCta") || "Go to Login"}
          />
        </Link>
      </div>
    </div>
          ...
          >
            ...
            {success ? (
              ...
              <>
                {/* Text */}
                <div
                  className={`px-5 sm:px-6 mt-9 sm:mt-10 ${
