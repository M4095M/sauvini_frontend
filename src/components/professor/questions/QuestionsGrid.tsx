...
export default function QuestionsGrid({
  ...
  const chapterOptions = useMemo(() => {
    ...
    questions.forEach((q) => q.chapter_name && set.add(q.chapter_name));
  ...
  const lessonOptions = useMemo(() => {
    const set = new Set<string>();
    questions.forEach((q) => q.module_name && set.add(q.module_name));
    const arr = Array.from(set);
    return arr.length ? ["All", ...arr] : ["All"];
  }, [questions]);
  ...
  const filtered = useMemo(() => {
    return questions.filter((q) => {
      if (chapterFilter !== "All" && q.chapter_name !== chapterFilter)
        return false;
      if (lessonFilter !== "All" && (q.module_name ?? "") !== lessonFilter)
        return false;
      if (statusFilter !== "All") {
        ...
        if (map[statusFilter] && q.question.status !== map[statusFilter])
          return false;
      }
      return true;
    });
  }, [questions, chapterFilter, lessonFilter, statusFilter]);
  ...
  return (
    ...
    >
      ...
      <div className="w-full mb-4 ">
        <div
          className={`w-full flex flex-col gap-3 md:flex-row md:items-center md:gap-3`}
        >
          <div className="relative w-full md:w-auto">
            <DropDown
              label=""
              placeholder={
                t("professor.questions.filter.selectLesson") ?? "Select lesson"
              }
          ...
          <div className="relative w-full md:w-auto">
            <DropDown
              label=""
              placeholder={
                t("professor.questions.filter.selectChapter") ??
                "Select chapter"
              }
          ...
          <div className="relative w-full md:w-auto">
            <DropDown
              label=""
              placeholder={
                t("professor.questions.filter.selectStatus") ?? "Select status"
              }
              options={statusOptions.map((s) => ({
                id: s,
                text: t(`professor.questions.status.${s.toLowerCase()}`) ?? s,
              }))}
              t={t}
              isRTL={isRTL}
              max_width="max-w-full"
            ...
            >
              {statusOptions.map((s) => (
                <option key={s} value={s}>
                  {t(`professor.questions.status.${s.toLowerCase()}`) ?? s}
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="flex flex-col gap-4 w-full ">
        {filtered.length === 0 ? (
          ...
          filtered.map((q) => (
            <div key={q.question.id} className="w-full ">
              <QuestionCard
                question={q}
                isRTL={isRTL}
                isMobile={isMobile}
