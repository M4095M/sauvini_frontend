...
export default function ProfessorCard({ professor, className = "" }: Props) {
  ...
  const primaryModule =
    professor.assignedModules?.[0]?.name || t("professor.general") || "General";

  // Display title based on role
  const titleText = isAdmin
    ? t("admin.title") || "Administrator"
  ...
  return (
    <div
      ...
      role="region"
      aria-label={`${isAdmin ? "" : t("professor.dr")} ${professor.firstName} ${
        professor.lastName
      } profile card`}
    >
      {/* Profile Picture - LTR */}
      ...
      {/* Content Section */}
      <div
        className={`flex-1 min-w-0 flex flex-col justify-center py-4 md:py-6 md:mx-8 ${
