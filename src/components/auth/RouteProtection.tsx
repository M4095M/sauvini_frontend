...
const DEFAULT_ROUTES: RouteConfig[] = [
  ...
  { path: "/debug-auth", requireAuth: false },
  { path: "/test-accept", requireAuth: false },
...
export function RouteProtection({
  ...
  const findMatchingRoute = useCallback(
    (path: string): RouteConfig | null => {
      // Special case for registration page
      if (path === "/register") {
        return {
          path: "/register",
          requireAuth: false,
      ...
      for (const route of routes) {
        if (
          ...
          path.startsWith(route.path + "?") ||
          path === route.path // Ensure exact match is also checked here
        ) {
          return route;
        }
      }
      ...
      return {
        path: path,
        requireAuth: false, // Default to public access for unknown routes
        allowedRoles: undefined,
      };
    },
    [routes]
  ...
  useEffect(() => {
    const checkRouteAccess = async () => {
      ...
      try {
        ...
        console.log("RouteProtection - matched route:", route);
        console.log("RouteProtection - requireAuth:", route?.requireAuth);

        // If no route found or route doesn't require auth, allow access immediately
        if (!route || !route.requireAuth) {
          console.log("RouteProtection - allowing access for public route");
          setIsAuthorized(true);
          setIsChecking(false);
          return;
        }

        // For protected routes, wait for auth context to finish loading
        if (isLoading) {
          console.log("RouteProtection - waiting for auth context to load");
          return;
        }
