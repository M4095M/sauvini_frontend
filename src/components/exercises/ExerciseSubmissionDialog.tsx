...
interface ExerciseSubmissionDialogProps {
  exerciseId: string
  onClose: () => void
  onSubmit: (data: { file: File; notes?: string }) => void | Promise<void>
}

export default function ExerciseSubmissionDialog({
  exerciseId,
  onClose,
  onSubmit,
}: ExerciseSubmissionDialogProps) {
  ...
  const inputId = `exercise-file-${exerciseId}`
  ...
  const handlePickedFile = (file?: File) => {
    if (!file) return
    if (!acceptPdf(file)) {
      alert(t("exercises.pdfOnlyError") || "Please select a PDF file")
      return
    }
    if (file.size > maxBytes) {
      alert(t("exercises.maxSizeError") || "File size must be under 10 MB")
      return
    }
  ...
  const handleSubmit = async () => {
    if (!selectedFile) {
      alert(t("exercises.selectFileError") || "Please select a PDF file")
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        <div className="mt-6">
          ...
          >
            {t("exercises.additionalNotes") || "Additional Notes"}
          </label>
          <textarea
            id={`notes-${exerciseId}`}
            value={notes}
            ...
            rows={6}
            placeholder={t("exercises.notesPlaceholder") || "Optional: Add any details for the reviewer..."}
            className={[
        ...
        <div className={`mt-6 grid grid-cols-2 gap-3 ${isRTL ? "direction-rtl" : ""}`}>
          ...
          <Button
            ...
            size="M"
            icon_position="none"
            text={isSubmitting ? (t("exercises.submitting") || "Submitting...") : (t("exercises.submit") || "Submit")}
