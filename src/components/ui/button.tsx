...
export default function Button({
  ...
  const setState = (state: string, size: string, disabled: boolean) => {
    switch (state) {
      case "filled":
        const filledClassName = `hover:bg-[#FFFFFF14] ${
          disabled
            ? "text-neutral-600 bg-neutral-200 cursor-not-allowed"
            : "bg-primary-300 text-neutral-100 cursor-pointer"
        }`;
        setClassName((prev) => `${prev} ${filledClassName}`);
        break;
  ...
  useEffect(() => {
    setState(state, size, disabled);
    setSize(size, icon_position);
    setIconPosition(icon_position);
    setLoading(false);
  }, [state, size, icon_position, disabled]);
