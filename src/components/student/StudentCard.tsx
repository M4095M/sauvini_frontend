...
function LevelBadge({ level, isRTL }: { level: number; isRTL?: boolean }) {
  return (
    <div
      aria-hidden
      className={`absolute -top-2 sm:-top-3 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-white rounded-full border border-neutral-200 shadow-sm ${
    ...
    >
      ...
      <span
        className="absolute text-[10px] sm:text-[11px] font-semibold text-white"
...
export default function StudentCard({ user, className = "" }: Props) {
  ...
  // Ensure we have a valid profile picture path
  const profilePicturePath =
    user.profile_picture_path && user.profile_picture_path !== "undefined"
      ? user.profile_picture_path
      : "/placeholder.svg";

  const imageSrc = profilePicturePath.startsWith("http")
    ? profilePicturePath
    : profilePicturePath.startsWith("/")
    ? `${url_prefix}${profilePicturePath}`
    : `${url_prefix}/${profilePicturePath}`;

  console.log("Profile picture src:", imageSrc);
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 w-full sm:h-[232px] flex flex-col justify-start py-4 sm:py-6 px-2 sm:px-4 lg:px-8 relative ${
          isRTL ? "items-end sm:items-end" : "items-start"
        }`}
      >
        <div className="w-full text-center sm:text-left">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-neutral-900 dark:text-white leading-tight">
            {user.first_name} {user.last_name ?? ""}
          </h2>
          <div className="mt-1 sm:mt-2 text-xs sm:text-sm text-neutral-500 dark:text-neutral-400">
            {user.academic_stream ?? "Academic Stream"}
          </div>
        </div>
        ...
        <div className="mt-4 sm:mt-6 flex items-center gap-3 sm:gap-6 lg:gap-8 w-full flex-wrap justify-center sm:justify-start">
          <Stat
            icon={
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-600 dark:text-neutral-200" />
            }
            value={(user as any).chaptersCompleted ?? 0}
            label="Chapter Completed"
            isRTL={isRTL}
          />
          <Stat
            icon={
              <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-600 dark:text-neutral-200" />
            }
            value={(user as any).lessonsCompleted ?? 0}
            label="Lesson Completed"
            isRTL={isRTL}
          />
          <Stat
            icon={
              <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-600 dark:text-neutral-200" />
            }
            value={(user as any).lessonsLeft ?? 0}
            label="Lesson Left"
            isRTL={isRTL}
          />
        </div>
