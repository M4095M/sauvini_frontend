    ...
    <div className="flex w-full max-w-[1152px] p-6 md:p-6 flex-col md:flex-row md:items-center gap-3 md:gap-0 rounded-[28px] 
      ...
      <div className={`hidden md:flex flex-row${isRTL ? "-reverse" : ""} justify-between items-center w-full`} >
        ...
        <div className={`flex items-start gap-3 ${isRTL ? "flex-row-reverse" : ""}`}>
          ...
          <div className="flex w-[30px] h-[30px] justify-center items-center rounded-full bg-[#CEDAE9] dark:bg-[#2A3A4A] flex-shrink-0">
            <span className={`text-sm font-medium text-[#1E40AF] dark:text-[#90B0E0] ${isRTL ? "font-arabic" : "font-sans"}`}>
              {lesson.order}
            </span>
          </div>
          ...
          <div
            className={`flex flex-col gap-1 max-w-[923px] ${isRTL ? "items-end text-right" : "items-start text-left"}`}
          >
      ...
      <div className="flex md:hidden flex-col items-center gap-3 w-full">
        ...
        <div className={`flex items-start gap-3 w-full ${isRTL ? "flex-row-reverse" : ""}`}>
          ...
          {/* Title and Description */}
          <div className={`flex flex-col gap-1 flex-1 ${isRTL ? "items-end text-right" : "items-start text-left"}`}>
            <h3 className={`text-lg font-semibold text-gray-900 dark:text-white ${isRTL ? "font-arabic" : "font-sans"}`}>
              {lesson.title}
            </h3>
            <p className={`text-sm text-gray-600 dark:text-gray-300 ${isRTL ? "font-arabic" : "font-sans"}`}>
