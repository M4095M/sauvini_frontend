...
interface ExamsCardProps {
  ...
  isRTL?: boolean;
  isMobile?: boolean;
  onCorrect?: (submissionId: string, payload?: { file?: File | null; mark?: number | null; notes?: string }) => void;
...
export default function ExamsCard({
  ...
  const correctionFileName =
    (submission.correctionFileUrl?.split("/").pop() ?? submission.correctionFileUrl ?? t("professor.exams.correctionFileDefault")) || "File.pdf";
  ...
  const handleSaveCorrection = async () => {
    ...
    onCorrect?.(submission.id, { file: correctionFile, mark: markNumber, notes });
  ...
  return (
    ...
    >
      {/* First frame */}
      <div className={`w-full flex flex-col md:items-center gap-4 ${firstFrameDirection}`}>
        <div className="flex items-start md:items-center gap-4 md:gap-6 flex-1">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className={`text-sm text-gray-600 dark:text-gray-300 ${textAlignClass}`}>
        ...
        <div className="mt-2 md:mt-0 md:flex-shrink-0 md:self-start md:ml-0">
          <div className={`w-full md:w-auto ${isRTL ? "md:text-left" : "md:text-right"}`}>
            <Button
              state="outlined"
              size={isMobile ? "S" : "M"}
              icon_position={downloadIconPosition}
              text={isMobile ? t("common.downloadShort") ?? "Download" : t("professor.exams.downloadSubmission") ?? "Download Student Submission file"}
      ...
      <div
        ...
        aria-hidden={!open}
      >
        <div ref={contentInnerRef} className={`pt-4 pb-2 ${detailsItemAlignment}`}>
          {/* Corrected expanded details */}
          {isCorrected ? (
            <div className="w-full flex flex-col gap-4">
              <div className={`flex flex-col gap-3 w-full ${detailsItemAlignment}`}>
                <div className={`text-2xl md:text-3xl font-semibold text-gray-900 dark:text-white ${textAlignClass}`}>
                ...
                {submission.professorNotes && (
                  <div className={`text-sm text-gray-700 dark:text-gray-300 ${textAlignClass}`}>
                    <strong className="block mb-1">{t("professor.exams.professorNotesTitle") ?? "Professor notes:"}</strong>
                    <div>{submission.professorNotes}</div>
                  </div>
                )}
              </div>

              <div className={`flex flex-col gap-2 w-full ${detailsItemAlignment}`}>
                <div className={`text-sm text-gray-600 dark:text-gray-400 ${textAlignClass}`}>{t("professor.exams.correctionFileLabel") ?? "Correction File"}</div>
                ...
                <div className="flex items-center justify-between w-full rounded-lg border border-[var(--BASE-Neutral-200,#EAEAEA)] p-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-sm text-gray-800 dark:text-gray-200">{correctionFileName}</div>
                  </div>
                  ...
                  <div className="flex-shrink-0">
                    <Button
                      ...
                      icon={<Download className="w-4 h-4" />}
                      onClick={() => submission.correctionFileUrl && window.open(submission.correctionFileUrl, "_blank")}
                      optionalStyles="!w-auto"
                    />
                  </div>
                </div>
            ...
            <div className="w-full flex flex-col gap-4">
              <div className="w-full">
                <AttachementField
                  label={t("professor.exams.uploadCorrectionLabel") ?? "Upload Correction File"}
                  max_size={50}
              ...
              <div className="w-full">
                <InputButton
                  label={t("professor.exams.studentTotalMarkLabel") ?? "Total Mark"}
                  type="plus-minus"
              ...
              <div className="w-full">
                <div className={`text-sm text-gray-600 dark:text-gray-400 mb-2 ${textAlignClass}`}>{t("professor.exams.additionalNotesLabel") ?? "Additional notes"}</div>
                <InputButton
                  label=""
                  type="icon"
                  icon_position={isRTL ? "left" : "right"}
              ...
              <div className="w-full flex justify-center">
                <Button state="filled" size="M" icon_position="left" text={t("professor.exams.saveCorrection") ?? "Save Correction"} icon={<Check className="w-4 h-4" />} onClick={handleSaveCorrection} />
              </div>
            </div>
          )}
        </div>
