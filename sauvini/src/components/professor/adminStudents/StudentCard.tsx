...
function Stat({
  ...
  return (
    <div className={`flex items-center gap-4 ${isRTL ? "flex-row-reverse" : "flex-row"}`}>
      ...
      {/* Number + label */}
      <div className={`flex flex-col ${isRTL ? "items-end" : "items-start"} flex-1`}>
        <div className="font-semibold text-lg" style={{ color: "var(--primary-300)" }}>
          {value}
        </div>
    ...
    >
      ...
      <div className={`mx-8 w-[768px] h-[232px] flex-shrink-0 flex flex-col justify-start py-6 relative ${isRTL ? "items-end" : "items-start"}`}>
        ...
        <div className="mt-6 flex items-center gap-8">
          ...
          <Stat
            isRTL={isRTL}
            icon={<FileText className="w-5 h-5 text-neutral-600 dark:text-neutral-200" />}
            value={student.lessonsCompleted ?? 0}
            label="Lesson Completed"
          />
          <Stat
            isRTL={isRTL}
            icon={<Clock className="w-5 h-5 text-neutral-600 dark:text-neutral-200" />}
