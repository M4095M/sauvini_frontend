...
export function AuthProvider({ 
  ...
  const handleApiError = useCallback((error: unknown): string => {
    ...
    if (typeof error === 'string') {
      ...
      } else if (typeof errorObj.response === 'object' && errorObj.response !== null) {
   ...
   */
  const registerStudent = useCallback(async (data: RegisterStudentData): Promise<User> => {
    setIsLoading(true);
    clearError();
   ...
   */
  const registerProfessor = useCallback(async (data: RegisterProfessorData): Promise<User> => {
    setIsLoading(true);
    clearError();
   ...
   */
  const hasRole = useCallback((role: 'student' | 'professor'): boolean => {
