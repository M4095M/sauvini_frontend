...
export class ProfessorApi extends BaseApi {
  ...
  static async updateProfile(profileData: {
    ...
    return this.put<Professor>('/professor/profile', profileData, { requiresAuth: true });
  }
   ...
   */
  static async updateCV(cvFile: File): Promise<ApiResponse<{
    cv_path: string;
  }>> {
    const formData = new FormData();
  ...
  static async changePassword(passwordData: {
    ...
    return this.post<null>('/professor/profile/change-password', passwordData, { requiresAuth: true });
  }
  ...
  static async getEnrolledStudents(
  ...
  ): Promise<ApiResponse<Array<Student & {
    ...
    const url = queryString ? `/professor/students?${queryString}` : '/professor/students';
    
    return this.get<Array<Student & {
      enrollment_date: string;
      progress: number;
      last_activity: string;
      completed_lessons: number;
   ...
   */
  static async getStudentProgress(studentId: string): Promise<ApiResponse<{
    student: Student;
    courses: Array<{
      course_id: string;
      course_title: string;
   ...
   */
  static async sendMessageToStudent(studentId: string, messageData: {
    subject: string;
    content: string;
    ...
    return this.post<{
      message_id: string;
      sent_at: string;
    }>(`/professor/students/${studentId}/message`, messageData, { requiresAuth: true });
  }
   ...
   */
  static async getCourses(pagination?: PaginationRequest): Promise<ApiResponse<Array<{
    id: string;
    title: string;
    description: string;
    created_at: string;
    ...
    const queryString = params.toString();
    const url = queryString ? `/professor/courses?${queryString}` : '/professor/courses';
   ...
   */
  static async updateCourse(courseId: string, updateData: {
    title?: string;
    description?: string;
    category?: string;
    difficulty_level?: 'beginner' | 'intermediate' | 'advanced';
  ...
  static async deleteCourse(courseId: string): Promise<ApiResponse<null>> {
    return this.delete<null>(`/professor/courses/${courseId}`, { requiresAuth: true });
  }
  ...
  static async getCourseLessons(courseId: string, pagination?: PaginationRequest): Promise<ApiResponse<Array<{
    ...
    const queryString = params.toString();
    const url = queryString ? `/professor/courses/${courseId}/lessons?${queryString}` : `/professor/courses/${courseId}/lessons`;
   ...
   */
  static async createLesson(courseId: string, lessonData: {
    title: string;
    description: string;
    content: string;
    order: number;
    ...
    return this.post<{
      ...
      is_published: boolean;
      created_at: string;
    }>(`/professor/courses/${courseId}/lessons`, lessonData, { requiresAuth: true });
  }
   ...
   */
  static async updateLesson(lessonId: string, updateData: {
    title?: string;
    description?: string;
    content?: string;
    order?: number;
  ...
  static async deleteLesson(lessonId: string): Promise<ApiResponse<null>> {
    return this.delete<null>(`/professor/lessons/${lessonId}`, { requiresAuth: true });
  }
   ...
   */
  static async createQuiz(lessonId: string, quizData: {
    title: string;
    description: string;
   ...
   */
  static async getQuizResults(quizId: string, pagination?: PaginationRequest): Promise<ApiResponse<{
    quiz: {
      id: string;
      title: string;
    ...
    const queryString = params.toString();
    const url = queryString ? `/professor/quizzes/${quizId}/results?${queryString}` : `/professor/quizzes/${quizId}/results`;
   ...
   */
  static async getTeachingAnalytics(period: 'week' | 'month' | 'quarter' | 'year' = 'month'): Promise<ApiResponse<{
    overview: {
   ...
   */
  static async exportCourseData(courseId: string, format: 'csv' | 'xlsx' | 'pdf' = 'csv'): Promise<ApiResponse<{
    download_url: string;
    expires_at: string;
    file_size: number;
  }>> {
    return this.post<{
      ...
      expires_at: string;
      file_size: number;
    }>(`/professor/courses/${courseId}/export`, { format }, { requiresAuth: true });
  }
}