...
export class AdminApi extends BaseApi {
   ...
   */
  static async getPendingProfessors(pagination?: PaginationRequest): Promise<ApiResponse<Professor[]>> {
   ...
   */
  static async getApprovedProfessors(pagination?: PaginationRequest): Promise<ApiResponse<Professor[]>> {
   ...
   */
  static async getRejectedProfessors(pagination?: PaginationRequest): Promise<ApiResponse<Professor[]>> {
   ...
   */
  static async getProfessor(professorId: string): Promise<ApiResponse<Professor>> {
    return this.get<Professor>(`/auth/admin/professors/${professorId}`, { requiresAuth: true });
  }
   ...
   */
  static async bulkApproveProfessors(professorIds: string[]): Promise<ApiResponse<{
    approved: string[];
    ...
    return this.post<{
      approved: string[];
      failed: Array<{ id: string; error: string }>;
    }>('/auth/admin/professors/bulk-approve', { ids: professorIds }, { requiresAuth: true });
  }
   ...
   */
  static async bulkRejectProfessors(professorIds: string[]): Promise<ApiResponse<{
    rejected: string[];
    failed: Array<{ id: string; error: string }>;
  }>> {
    return this.post<{
      rejected: string[];
      failed: Array<{ id: string; error: string }>;
    }>('/auth/admin/professors/bulk-reject', { ids: professorIds }, { requiresAuth: true });
  }
  ...
  static async getAllStudents(
    ...
    const queryString = params.toString();
    const url = queryString ? `/auth/admin/all-students?${queryString}` : '/auth/admin/all-students';
  ...
  static async getStudent(studentId: string): Promise<ApiResponse<Student>> {
    return this.get<Student>(`/auth/admin/students/${studentId}`, { requiresAuth: true });
  }
   ...
   */
  static async suspendStudent(studentId: string, reason?: string): Promise<ApiResponse<null>> {
    return this.post<null>(`/auth/admin/students/${studentId}/suspend`, { reason }, { requiresAuth: true });
  }
   ...
   */
  static async reactivateStudent(studentId: string): Promise<ApiResponse<null>> {
    return this.post<null>(`/auth/admin/students/${studentId}/reactivate`, {}, { requiresAuth: true });
  }
   ...
   */
  static async getAllAdmins(pagination?: PaginationRequest): Promise<ApiResponse<Admin[]>> {
    const params = new URLSearchParams();
    if (pagination?.page) params.append('page', pagination.page.toString());
    if (pagination?.limit) params.append('limit', pagination.limit.toString());
    
    const queryString = params.toString();
    const url = queryString ? `/auth/admin/all-admins?${queryString}` : '/auth/admin/all-admins';
  ...
  static async createAdmin(adminData: {
    ...
    return this.post<Admin>('/auth/admin/create-admin', adminData, { requiresAuth: true });
  }
  ...
  static async updateAdmin(adminId: string, updateData: {
    ...
    return this.put<Admin>(`/auth/admin/admins/${adminId}`, updateData, { requiresAuth: true });
  }
  ...
  static async deleteAdmin(adminId: string): Promise<ApiResponse<null>> {
    return this.delete<null>(`/auth/admin/admins/${adminId}`, { requiresAuth: true });
  }
   ...
   */
  static async getUserActivityAnalytics(period: 'day' | 'week' | 'month' | 'year' = 'week'): Promise<ApiResponse<{
    registrations: Array<{ date: string; students: number; professors: number }>;
    ...
    return this.get<{
      ...
      logins: Array<{ date: string; count: number }>;
      active_users: Array<{ date: string; count: number }>;
    }>(`/auth/admin/analytics/activity?period=${period}`, { requiresAuth: true });
  }
  ...
  static async getSystemHealth(): Promise<ApiResponse<{
    ...
    memory_usage: { used_mb: number; available_mb: number; percentage: number };
   ...
   */
  static async clearSystemCache(cache_type: 'all' | 'user_sessions' | 'file_cache' | 'redis' = 'all'): Promise<ApiResponse<{
    cleared: string[];
    errors: string[];
  }>> {
    return this.post<{
      cleared: string[];
      errors: string[];
    }>('/auth/admin/system/clear-cache', { cache_type }, { requiresAuth: true });
  }
   ...
   */
  static async updateSystemConfig(config: Record<string, unknown>): Promise<ApiResponse<Record<string, unknown>>> {
    return this.put<Record<string, unknown>>('/auth/admin/system/config', config, { requiresAuth: true });
  }
}