...
export function useAdmin() {
  ...
  const getPendingProfessors = useCallback(async (pagination?: PaginationRequest) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getApprovedProfessors = useCallback(async (pagination?: PaginationRequest) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getRejectedProfessors = useCallback(async (pagination?: PaginationRequest) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const suspendStudent = useCallback(async (studentId: string, reason?: string): Promise<boolean> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const reactivateStudent = useCallback(async (studentId: string): Promise<boolean> => {
    setLoading(true);
    setError(null);
    try {
  ...
  const createAdmin = useCallback(async (adminData: { email: string; password: string }) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateAdmin = useCallback(async (adminId: string, updateData: { email?: string; password?: string }) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const getUserActivityAnalytics = useCallback(async (period: 'day' | 'week' | 'month' | 'year' = 'week') => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (response.success && response.data) {
        ...
        setError(response.message || 'Failed to fetch user activity analytics');
        return null;
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unexpected error occurred';
  ...
  const clearSystemCache = useCallback(async (cache_type: 'all' | 'user_sessions' | 'file_cache' | 'redis' = 'all') => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateSystemConfig = useCallback(async (config: Record<string, unknown>) => {
    setLoading(true);
    setError(null);
    try {
